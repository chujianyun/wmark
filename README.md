...
字体下拉框自动读取系统字体目录中的已安装字体，支持独立切换加粗；所选字体随模板和上次设置保存。安装新字体后重启应用即可使用。内置思源黑体 Noto Sans CJK SC 作为默认及缺字回退字体，字体不可用时也回退到思源黑体，授权见 `assets/fonts/OFL.txt`。未标记色彩空间的输入按 sRGB 解释；RGB ICC 输入转换至 sRGB。非 RGB ICC 输入提示先转换。WebP 输出无损，质量滑块只作用于 JPG。水印不能保证防止盗图或滥用。
